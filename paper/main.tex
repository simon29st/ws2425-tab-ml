...
As there is no clear definition, we will consider ``interpretability'' as ``the ability to provide \textit{explanations} in \textit{understandable terms} to a human'',
...
We use the term ``explanability'' in an exchangeable manner with ``interpretability'', as is commonly done.
...
- in many ways desirable, as per \citet[pp. 3-4]{Zach2019InterpretabilityOD}, e.g.
  ...
  * \textbf{model debugging}: model is optimised w.r.t. loss and judged on accuracy, having an interpretable model + model induction process can help find ways to improve accuracy
  * \textbf{scientific understanding}: when only models can make sense of increasingly complex data, interpretability enables extraction of learnt knowledge encoded in the model
  * \textbf{subconcious bias}, e.g. loan approval: must ensure that decision is not discriminatory
  * \textbf{regulatory}, such as EU's "Right to Explanation" warranted by the GDPR \citep[p. 1]{review_NN_interpretability}
...
(Taxonomy)
As standard neural networks (NNs) are inherently non-interpretable models, the taxonomy introduced by \citet[chap. 2]{survey_NN_interpretability} proves particulary useful
to categorise NN explanability methods along 3 dimensions:
