...
%\blindtext
- propose a network architecture specifically suited for the EAGGA algorithm
- achieve comparable performance as training XGBoost with EAGGA
\end{abstract}
  ...
  * neural network
    ...
    - NN
      ...
      * hidden layers use ReLU activation and dropout afterwards, then a shared output layer with concatenated sub-NNs' outputs as input and sigmoid activation with
         binary cross entropy loss (sigmoid implicit in the loss function for better numerical stability, NN itself outputs "logits", which refers to pre-activation output in pytorch)
...
\begin{figure}
  \centering
  \includegraphics[width=0.9\linewidth]{../code/export/plot_test_dhvs.png}  % scale figure, cf. https://tex.stackexchange.com/a/16584
  \caption{caption}
  \label{fig-test-dhvs}
\end{figure}
...
- dhv over generations Figure \ref{fig-val-dhvs}
  ...
  * artifacts / drops along y likely due to inconsistent computation of Pareto front by third-party library
    - preliminary experiments on dummy pareto fronts: noticed nds function returning different rankings for same front
    - returned ranking switched back and forth between two slightly different options (only 1 or 2 indices were swapped)
  ...
  * BUT: absolute as well as improvement almost negligible -> no large dhv drop would we just evaluate the models gotten from the detectors
...
\begin{figure}
  ...
  \caption{Pareto set loss of all datasets, evaluated on early stopping set. Same colours denote losses come from folds of the same individual.
